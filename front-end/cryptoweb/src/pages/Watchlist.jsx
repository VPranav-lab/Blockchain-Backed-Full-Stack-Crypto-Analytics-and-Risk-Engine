...
export default function Watchlist() {
  ...
  const handleRemove = async (symbol) => {
    ...
    try { 
      ...
      // Only revert if it's a critical error (Network/500)
      // 404s are ignored by our API wrapper, so this won't trigger for them.
      setWatchlist(previousList);
      alert("Could not delete coin. Check network connection.");
    }
  };
      ...
      <div className={styles.header}>
        ...
        <div className={styles.searchContainer} ref={searchRef}>
          <div className={styles.inputWrapper}>
            ...
            <input 
              type="text" placeholder="Search coin..." className={styles.searchInput}
              value={searchTerm}
              ...
              {searchResults.length > 0 ? (
                searchResults.map(coin => (
                  <div key={coin.symbol} className={styles.resultItem} onClick={() => handleAdd(coin.symbol)}>
                    ...
                    <span className={styles.resPrice}>${fmtPrice(coin.current_price || 0)}</span>
                  </div>
                ))
                ...
                <tr key={row.symbol} className={styles.row}>
                  <td>
                    <div className={styles.asset}>
                      {row.symbol}<span className={styles.badge}>{row.base} Network</span>
                    </div>
                  </td>
                  <td className={`${styles.tdRight} ${styles.mono}`}>${fmtPrice(row.price)}</td>
                  <td className={`${styles.tdRight} ${styles.mono}`}>
                    <span className={row.change >= 0 ? styles.green : styles.red}>{row.change >= 0 ? "+" : ""}{row.change.toFixed(2)}%</span>
                  </td>
                  <td className={`${styles.tdRight} ${styles.dim}`}>{fmtQty(row.volume)}</td>
                  <td className={styles.tdRight}>
                    <div className={styles.actions}>
                      <button className={styles.tradeBtn} onClick={() => navigate(`/trades?symbol=${row.symbol}`)}>Trade</button>
                      
                      {/* ✅ FIX: Remove using SYMBOL */}
                      <button className={styles.removeBtn} onClick={() => handleRemove(row.symbol)}>✕</button>
                    </div>
                  </td>
                </tr>
              ))
